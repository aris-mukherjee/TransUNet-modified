   ...
   # print(f"Unique2: {np.unique(gttr)}")
    ...
    for epoch_num in iterator:
        ...
        for sampled_batch in iterate_minibatches(args, imtr, gttr, batch_size = exp_config.batch_size, train_or_eval = 'train'):
            ...
            #utils.save_nii(img_path ='/scratch_net/biwidl217_second/arismu/Data_MT/' + 'FETS_train.nii.gz', data = image_batch, affine = np.eye(4))
            #utils.save_nii(img_path ='/scratch_net/biwidl217_second/arismu/Data_MT/' + 'LABEL_FETS_train.nii.gz', data = label_batch, affine = np.eye(4))
