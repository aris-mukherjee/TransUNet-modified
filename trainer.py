...
def trainer_runmc(args, model, snapshot_path):
    ...
    writer = SummaryWriter('/scratch_net/biwidl217_second/arismu/Tensorboard/2022/' + 'TU_NO_QUERY_log_400epochs_seed1234') 
    iter_num = 0
    ...
    # ============================  
            ...
            if (iter_num+1) % 85 == 0:
                ...
                if val_loss < best_val_loss:
                    save_mode_path = os.path.join('/scratch_net/biwidl217_second/arismu/Master_Thesis_Codes/project_TransUNet/model/2022/', 'TU_NO_QUERY_best_val_loss_seed1234' + '.pth')
    ...
    # ============================  
        ...
        if epoch_num > int(max_epoch / 4) and (epoch_num + 1) % save_interval == 0:
            save_mode_path = os.path.join('/scratch_net/biwidl217_second/arismu/Master_Thesis_Codes/project_TransUNet/model/2022/', 'TU_NO_QUERY_best_val_loss_seed2234' + 'epoch_' + str(epoch_num) + '.pth')
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
        if epoch_num >= max_epoch - 1:
            save_mode_path = os.path.join('/scratch_net/biwidl217_second/arismu/Master_Thesis_Codes/project_TransUNet/model/2022/', 'TU_NO_QUERY_best_val_loss_seed1234' + 'epoch_' + str(epoch_num) + '.pth')
            torch.save(model.state_dict(), save_mode_path)
            logging.info("save model to {}".format(save_mode_path))
            iterator.close() 
             """
